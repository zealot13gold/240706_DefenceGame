...
public class EnemyIdleState : IState
{
    ...
    {
        Debug.LogFormat("EnemyIdleState: 플레이어 유닛 검색");
        ...
        {
            Debug.LogFormat("EnemyIdleState: {0}이 {1}을 발견", unit.name, sm.players[i].gameObject.name);
            ...
            if (bufferPlayerDist > Mathf.Abs((bufferPlayerPos - unit.transform.position).magnitude))                    // 현재 위치로부터 가장 가까운 공격 대상을 찾음
            {
                ...
                Debug.LogFormat("EnemyIdleState: {0}의 타겟은 {1}", unit.name, targetPlayer.name);
